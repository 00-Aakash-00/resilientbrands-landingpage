      ...
      >
        <div className={`${isMobile ? "mt-24 px-2" : "mt-32"}`}>
          <motion.span
            ...
            className={`inline-block text-cyan-400 ${isSmallMobile ? "text-sm" : isMobile ? "text-base" : "text-lg"} font-medium mb-6`}
          >
            Become a Solution
          </motion.span>
          ...
          >
            You&apos;ve built a solution that delivers real outcomes — often more advanced than anything on the market. But breaking through crowded, algorithm-driven channels is harder than ever.
          </motion.p>
          ...
          >
            <Button
              ...
              className={`bg-gradient-to-r from-cyan-500 to-emerald-500 hover:from-cyan-600 hover:to-emerald-600 text-white font-semibold ${isMobile ? "text-base px-6 py-2" : "text-lg px-8 py-3"} h-auto transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-cyan-500/25`}
              onClick={openContactModal}
            >
              Join Resilient Brands Today
              <ArrowRight className={`${isMobile ? "ml-2 h-4 w-4" : "ml-3 h-5 w-5"}`} />
            </Button>
          </motion.div>
        </div>
      </div>
